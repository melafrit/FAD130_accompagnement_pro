...
export default function Entretien() {
  ...
  async function startSession(dossierId: number) {
    const r = await api<{ sessionId: number }>('/entretien/sessions', { method: 'POST', body: JSON.stringify({ dossierId }) })
    ...
    s.reponses.forEach((x) => {
      map[Number(x.phase)] = x.texte_reponse
    })
    setNotes(map)
  ...
  async function askIA() {
    ...
    setBusy(true)
    try {
      const s = await api<Suggestion>('/entretien/suggestions', { method: 'POST', body: JSON.stringify({ phase: current, transcript: notes[current] || '' }) })
      setSugg(s)
    } finally {
      setBusy(false)
    }
  ...
  if (sessionId == null) {
    return (
      <div className="page">
        ...
        {dossiers.length === 0 && (
          <p className="muted">Aucun accompagné pour l'instant. Un dossier est créé automatiquement quand un accompagné complète son questionnaire initial.</p>
        )}
        <div className="cards">
          {dossiers.map((d) => (
  ...
  return (
    <div className="page">
      ...
      <div className="phase-steps">
        {phases.map((p, i) => (
          <button key={p.id} className={`phase-step ${i === current ? 'active' : ''}`} onClick={() => goTo(i)} aria-label={`Phase ${i}`}>{i}</button>
        ))}
      </div>
      ...
      <div className="notes-block">
        ...
        <div className="notes-actions">
          <button className="btn btn-primary" disabled={busy} onClick={askIA}>{busy ? '…' : '✨ Suggestions de l’IA'}</button>
        </div>
        {sugg && (
          <div className="sugg">
      ...
      <div className="entretien-nav">
        ...
        {current < phases.length - 1 ? (
          <button className="btn btn-primary" onClick={() => goTo(current + 1)}>Suivant →</button>
        ) : (
          <button className="btn btn-primary" onClick={terminer}>Terminer l’entretien</button>
        )}
      </div>
    </div>
  )
